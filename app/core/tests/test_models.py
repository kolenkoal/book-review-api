...
class ModelTests(TestCase):
    ...
    def test_create_author(self):
        ...
        author = models.Author.objects.create(
            user=user, first_name="Ivan", second_name="Minin"
        )

        self.assertEqual(
            str(author), author.first_name + " " + author.second_name
        )

    def test_create_genre(self):
        """Test creating a user."""
        user = create_user(email="user2@example.com", password="testpass123")
        genre = models.Genre.objects.create(user=user, name="Fantasy")
